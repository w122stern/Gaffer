...
public abstract class PropertiesSerialiser {
    ...
    protected byte[] serialiseProperties(final Properties properties, final String group) throws SerialisationException {
        ...
        try {
            writeBytes(StringUtil.toBytes(group), out);
        } catch (IOException e) {
    ...
    protected int getLastDelimiter(final byte[] allBytes, final byte[] fieldBytes, final int lastDelimiter) {
        return lastDelimiter + CompactRawSerialisationUtils.decodeVIntSize(allBytes[lastDelimiter]) + fieldBytes.length;
    }

    protected byte[] getFieldBytes(final byte[] bytes, final int startPos) throws SerialisationException {
    ...
    protected void writeBytes(final byte[] bytes, final ByteArrayOutputStream out)
            throws IOException {
